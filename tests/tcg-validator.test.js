...
function buildMinimalZip() {
  ...
  zip.file('cards.json', JSON.stringify([
    { id: 1, type: 1, level: 3, rarity: 1, atk: 1000, def: 800, attribute: 3, race: 1, name: 'Test Card', description: 'A test card' },
  ]));
  zip.folder('img');
...
describe('validateTcgArchive', () => {
  ...
  it('accepts archive without cards_description.json (v2 uses inline fields)', async () => {
    const zip = buildMinimalZip();
    const result = await validateTcgArchive(zip);
    expect(result.valid).toBe(true);
  });
  ...
  it('warns on card missing inline name and description', async () => {
    const zip = buildMinimalZip();
    zip.file('cards.json', JSON.stringify([
      { id: 1, type: 1, level: 3, rarity: 1, atk: 1000, def: 800, attribute: 3, race: 1, name: 'Test Card', description: 'A test card' },
  ...
  it('accepts archive with legacy cards_description.json (ignored in v2)', async () => {
    const zip = buildMinimalZip();
